...
export function createReleaseSignup({
  ...
  return async (request, context = {}) => {
    ...
    if (!allowedOrigin(request, context)) {
      return json({ ok: false, message: "Please sign up through the Lockliel website." }, 403);
    }
    ...
    try {
      ...
      if (!firstName || firstName.length > 80 || /[<>\x00-\x1f]/.test(firstName)) {
        throw Error("Please enter your first name.");
      }
      if (email.length > 254 || !/^[^\s<>@]+@[^\s<>@]+\.[^\s<>@]+$/.test(email)) {
        throw Error("Please enter a valid email address.");
      }
      if (data.releaseConsent !== true || data.botField) {
        throw Error("Please use the release-notification form to sign up.");
      }
      ...
      lead = {
        ...
        releaseConsent: true,
        consentText,
        consentVersion: "heart-release-v1",
    ...
    try {
      ...
      const key = "heart-for-the-lost/leads/" + hash(lead.email);
      const syncKey = "heart-for-the-lost/form-sync/" + hash(lead.email);
      ...
      const retry = !sync || (
        !sync.data.sent &&
        (!sync.data.pending || Date.now() - sync.data.startedAt > 60000)
      );
      const claimed = retry && (
        await store.setJSON(
          syncKey,
          { sent: false, pending: true, startedAt: Date.now() },
          sync ? { onlyIfMatch: sync.etag } : { onlyIfNew: true }
        )
      ).modified;
      ...
      if (claimed) {
        try {
          const fields = new URLSearchParams({
            "form-name": "heart-for-the-lost-release",
            "first-name": saved.firstName,
            email: saved.email,
            book,
            "signup-at": saved.signupAt,
            "release-consent": "yes",
            "consent-version": saved.consentVersion,
            ...saved.attribution
          });

          const response = await post("https://lockliel.com/__heart-book-release.html", {
            method: "POST",
          ...
          await store.setJSON(syncKey, { sent: false, at: new Date().toISOString() });
        }
      }
      ...
      try {
        await fetch(SUPABASE_URL + "/functions/v1/capture-lead", {
          ...
          body: JSON.stringify({
            ...
            campaign: "a-heart-for-the-lost-release",
            attribution: saved.attribution,
            consent: {
              releaseNotifications: true,
              generalMarketing: false,
              version: saved.consentVersion,
              text: saved.consentText
            }
          }),
          signal: AbortSignal.timeout(5000)
        });
      } catch {
        // CRM mirroring must not block the release signup.
      }
      ...
      return json({
        ok: false,
        message: "We couldn’t confirm your signup. Your details are still here. Please try again, or contact info@lockliel.com."
      }, 503);
    }
  };
}
