...
export default async(request)=>{
  ...
  const [flagsRes,providersRes,courseRes,assetsRes,productsRes,rolesRes]=await Promise.all([
    ...
    fetch(SUPABASE_URL+"/rest/v1/lesson_assets?select=id,lesson_id,asset_type,status,provider,storage_path&limit=1000",{headers:h}),
  ...
  const checks=[
    ...
    },
    {
      ...
      ready:hasSuperAdmin,
      detail:hasSuperAdmin?"A super administrator account is assigned.":"Create the first real member account, then assign its staff role to super_admin directly in Supabase."
    },
    {
      key:"course",
    ...
    },
    {
      ...
      detail:digitalBook?.storage_path?"Protected PDF is uploaded.":"Digital product exists, but the final protected PDF still needs to be uploaded."
    },
    {
      key:"giving_provider",
      label:"Live giving processor",
      ready:Boolean(activeProvider),
      detail:activeProvider?activeProvider.label+" is active.":"No payment provider is active yet. Authorize.Net, Stripe, PayPal and Square are modeled but not connected."
    },
    {
      key:"partner_checkout",
      label:"Live partner checkout switch",
      ready:Boolean(flagMap.partner_checkout)&&Boolean(activeProvider),
      detail:flagMap.partner_checkout?"Feature flag is on.":"Feature flag remains off until the processor is connected and tested."
    },
    {
      key:"book_benefit",
      label:"$20 book benefit",
      ready:Boolean(flagMap.heart_book_gift_benefit)&&Boolean(digitalBook?.storage_path),
      detail:flagMap.heart_book_gift_benefit?"Benefit is active.":"Benefit automation is built but intentionally not activated."
    },
    {
      key:"founders50",
      label:"Founders 50 intake",
      ready:Boolean(flagMap.founders50_public_recruiting),
      detail:flagMap.founders50_public_recruiting?"Applications are operationally open.":"Founders 50 recruiting is currently paused."
    },
    {
      key:"messaging",
      label:"Private internal messaging",
      ready:Boolean(flagMap.internal_messaging),
      detail:flagMap.internal_messaging?"Inviter and assigned-leader messaging is enabled.":"Messaging is currently disabled."
    }
  ];
