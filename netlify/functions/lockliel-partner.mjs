...
export default async(request)=>{
 ...
 const [pr,cr,gr,br,fr]=await Promise.all([
  ...
  fetch(SUPABASE_URL+"/rest/v1/benefit_rules?status=eq.active&select=slug,title,minimum_gift_cents,fulfillment_type",{headers:h}),\n  fetch(SUPABASE_URL+"/rest/v1/feature_flags?key=in.(partner_checkout,heart_book_gift_benefit)&select=key,enabled",{headers:h})
 ]);
 const providers=pr.ok?await pr.json():[],commitments=cr.ok?await cr.json():[],gifts=gr.ok?await gr.json():[],benefits=br.ok?await br.json():[],flags=fr.ok?await fr.json():[];\n const flagMap=Object.fromEntries(flags.map(x=>[x.key,x.enabled]));
 const totalGiven=gifts.filter(g=>["succeeded","paid","completed"].includes(g.status)).reduce((sum,g)=>sum+Number(g.amount_cents||0),0);
 return json({providers,commitments,gifts,benefits,totalGiven,flags:flagMap,checkoutReady:Boolean(flagMap.partner_checkout)&&providers.some(p=>p.status==="active")},200,s.refreshed?sessionCookies(s.refreshed):[]);
};
