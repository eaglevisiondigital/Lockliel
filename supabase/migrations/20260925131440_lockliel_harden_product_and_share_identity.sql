revoke update on table public.products
from authenticated;
...
alter table public.share_assets
  drop constraint if exists share_assets_status_check,
  add constraint share_assets_status_check
...
alter table public.share_assets
  ...
  add constraint share_assets_share_text_length
    check (
      share_text is null
      or char_length(share_text)<=1200
    );
...
alter table public.share_assets
  ...
  add constraint share_assets_preview_path_length
    check (
      preview_image_path is null
      or char_length(preview_image_path)<=500
    );
...
revoke insert, update on table public.share_assets
from authenticated;
