create or replace function public.lockliel_assign_group_member(
  group_uuid uuid,
  member_uuid uuid,
...
begin
  ...
  if group_uuid is null or member_uuid is null then
  ...
  if group_status is null
     or group_status not in ('forming','active') then
  ...
  if request_uuid is not null then
    ...
    if not found then
      raise exception 'Group request changed before it could be resolved.';
    end if;
  end if;
...
revoke execute on function public.lockliel_assign_group_member(uuid,uuid,uuid)
from public, anon;

grant execute on function public.lockliel_assign_group_member(uuid,uuid,uuid)
to authenticated;
