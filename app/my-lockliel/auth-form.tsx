...
export default function LocklielAuthForm({ mode }: { mode: "login" | "signup" }) {
  ...
  return <form className="ml-auth-card" onSubmit={submit}>
    ...
    {referralCode&&mode==="signup"&&<p className="ml-referral-note">You arrived through a personal Lockliel invitation. We’ll preserve that connection when your account is created.</p>}\n    {mode==="signup"&&<p className="ml-privacy-note">If you joined through a personal invitation, your inviter may see that you joined and can message you inside My Lockliel. We do not give them your email address or phone number.</p>}\n    {mode==="login"&&<p className="ml-auth-help"><Link href="/my-lockliel/forgot-password">Forgot your password?</Link></p>}
