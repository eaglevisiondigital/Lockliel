...
export default function ForgotPasswordForm(){
 ...
 return <form className="ml-auth-card" onSubmit={submit}><label>Email address<input name="email" type="email" autoComplete="email" required/></label>{message&&<p className={status==="error"?"ml-auth-message error":"ml-auth-message"}>{message}</p>}{status!=="sent"&&<button className="ml-action" disabled={status==="sending"}>{status==="sending"?"Sending…":"Send reset link"}</button>}</form>;
}