...
import { BookOpen, CheckCircle2, ClipboardCheck, Eye, HeartHandshake, Share2, ShieldCheck, Sprout, UserPlus, Users } from "lucide-react";
import { useEffect,useState } from "react";

type AdminData={roles:string[];counts:{people:number;founders:number;activeCourses:number;gifts:number;leads:number;followups:number};activity:{share_initiated:number;visit:number;signup:number;course_started:number;lesson_completed:number};sourceCounts:Record<string,number>;applications:any[]};
...
export default function AdminClient(){
 ...
 const stats=[["People",data.counts.people,Users],["Pre-account leads",data.counts.leads,UserPlus],["Founders 50",data.counts.founders,ShieldCheck],["Active courses",data.counts.activeCourses,BookOpen],["Open follow-ups",data.counts.followups,ClipboardCheck],["Recorded gifts",data.counts.gifts,HeartHandshake]];
